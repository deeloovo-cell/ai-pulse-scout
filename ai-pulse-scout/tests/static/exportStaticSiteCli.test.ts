...
describe('capStaticDigestItems', () => {
  it('keeps the top 100 ordered items for static digest export', () => {
    const items = Array.from({ length: 110 }, (_, index) => ({ id: `item-${index + 1}` }));

    expect(capStaticDigestItems(items)).toHaveLength(100);
